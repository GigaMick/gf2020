@extends('../layouts.app')

@section('content')
    <section class="min-vh-100">
        <div class="container">
            <div class="row min-vh-80">
                <div class="col-12 col-md-4 offset-md-1 d-flex align-items-center justify-content-between flex-row">
                    <div class="">
                        <h1 class="fw700"><a href="/cook/basic-details" class="signpost">Sign up to cook</a></h1>
                    </div>
                    <div class="">
                        <h1 class="">-></h1>
                    </div>
                </div>
                <div class="col-12 col-md-4 d-flex offset-md-2 align-items-center justify-content-between flex-row">
                        <div class="">
                            <h1 class="fw700"><a href="/customer/basic-details" class="signpost">Sign up to eat</a></h1>
                        </div>
                        <div class="">
                            <h1 class="">-></h1>
                        </div>
                </div>
            </div>
        </div>
    </section>
@endsection